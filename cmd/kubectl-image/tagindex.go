...
// imageindex provides identification for a single image. Here 'server' is the
// imgctrl url (i.e. the cluster), namespace and name uniquely identify an image
// in the cluster.
...
func (t imageindex) containerRuntime() (int, error) {
...
func (t imageindex) localStorageRef() (types.ImageReference, error) {
...
// indexFor receives a path to an image hosted at a imgctrl instance
...
func indexFor(ipath string) (imageindex, error) {
	...
	// user has requested an image by its hash, we don't care about
	// the hash part, we only care about the namespace and the name.
	...
	// like "imgctrl.addr:port/namespace/name:tag", this "tag" portion
